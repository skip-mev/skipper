...
async def main():
    ...
    config: Config = Config(env_file_path=ENV_FILE_PATH)
    querier = QuerierFactory.create(impl=config.querier, rpc_url=config.rpc_url)
    decoder = DecoderFactory.create(impl=config.decoder)
